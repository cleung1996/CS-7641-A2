...
def sa():
    ...
    for n in num_queens:
        problem = mlrose_hiive.DiscreteOpt(length=n, fitness_fn=fitness_cust, maximize=True, max_val=n)
    ...
    for result in best_fitness_sa_time:
        print(f"Best Fitness Score for N = {num_queens[counter]} - SA")
        ...
        plot_curve_avg(fitness_curves, f"{num_queens[counter]} Queens", f"{num_queens[counter]}_Queens_Fit_Iter_SA_new", "SA")
        counter += 1
...
def rhc():
    fitness_cust = mlrose_hiive.CustomFitness(queens_max)
    num_queens = [5,10,50,100]
    max_iterations = [50, 100, 500, 1000, 5000]
    max_attempts = [5, 10, 20, 30, 50]
    ...
    for result in best_fitness_rhc_time:
        print(f"Best Fitness Score for N = {num_queens[counter]} - RHC")
        print( f"Params = Fitness: {result[0]}, Iteration: {result[1]}, Restart: {result[2]}, Attempt: {result[3]}, Diff: {result[4]}, FeVals Avg: {mean(np.mean(result[5], axis=0))}")

        fitness_curves = result[-1]

        plot_curve_avg(fitness_curves, f"{num_queens[counter]} Queens", f"{num_queens[counter]}_Queens_Fit_Iter_RHC", "RHC")
        counter += 1
...
def ga():
    ...
    for result in best_fitness_ga_time:
        print(f"Best Fitness Score for N = {num_queens[counter]} - GA")
        print(
        ...
        plot_curve_avg(fitness_curves, f"{num_queens[counter]} Queens", f"{num_queens[counter]}_Queens_Fit_Iter_GA_new",
                       "GA")
        counter += 1
...
def rhc_param_tune():
    ...
    for attempt in max_attempts:
        problem = mlrose_hiive.DiscreteOpt(length=num_queens, fitness_fn=fitness_cust, maximize=True, max_val=num_queens)
    ...
    plt.title(f'RHC - {num_queens} Queens - Modifying Max Attempts', fontsize=10)
    plt.legend()
    plt.xlabel('Iterations')
    plt.ylabel('Fitness Score')
    plt.savefig('Queens_50_Max_Attempt_Tune_RHC.png')
    plt.show()
    plt.close()
    ...
    plt.title(f'RHC - {num_queens} Queens - Modifying Restarts', fontsize=10)
    plt.legend()
    plt.xlabel('Iterations')
    plt.ylabel('Fitness Score')
    plt.savefig('Queens_50_Restarts_Tune_RHC.png')
    plt.show()
...
def sa_param_tune():
    ...
    problem = mlrose_hiive.DiscreteOpt(length=num_queens, fitness_fn=fitness_cust, maximize=True,
                                       max_val=num_queens)
    plt.figure()
    ...
    best_state, best_fitness, fitness_curve = mlrose_hiive.simulated_annealing(problem=problem,
                                                                                   ...
                                                                                   curve=True)
    first_col = fitness_curve[:, 0]
    plt.plot(first_col, label=f"Schedule=GeomDecay")

    plt.title(f'RHC - {num_queens} Queens - Modifying Decay Schedules', fontsize=10)
    plt.legend()
    plt.xlabel('Iterations')
    plt.ylabel('Fitness Score')
    plt.savefig('Queens_50_Schedule_Tune_SA.png')
    plt.show()
    plt.close()
    plt.figure()
    ...
    # plt.xlabel('Iterations')
    # plt.ylabel('Fitness Score')
    # plt.savefig('Queens_50_Max_Attempt_Tune_SA.png')
    # plt.show()
    ...
    #
    # plt.title(f'RHC - {num_queens} Queens - Modifying Initial Temp', fontsize=10)
    # plt.legend()
    # plt.xlabel('Iterations')
    # plt.ylabel('Fitness Score')
    # plt.savefig('Queens_50_Init_T_Tune_SA.png')
    # plt.show()
...
def ga_param_tune():
    ...
    plt.title(f'GA - {num_queens} Queens - Modifying Max Attempts', fontsize=10)
    plt.legend()
    plt.xlabel('Iterations')
    plt.ylabel('Fitness Score')
    plt.savefig('Queens_50_Max_Attempt_Tune_GA.png')
    plt.show()
    plt.close()
    ...
    plt.title(f'GA - {num_queens} Queens - Modifying Mutations', fontsize=10)
    plt.legend()
    plt.xlabel('Iterations')
    plt.ylabel('Fitness Score')
    plt.savefig('Queens_50_Mutation_Prob_Tune_GA.png')
    plt.show()
    plt.close()
    ...
    plt.title(f'GA - {num_queens} Queens - Modifying Population', fontsize=10)
    plt.legend()
    plt.xlabel('Iterations')
    plt.ylabel('Fitness Score')
    plt.savefig('Queens_50_Pop_Size_Tune_GA.png')
    plt.show()
    plt.close()
...
#     # RHC     Best Params = [Fitness : 4927, Iterations : 1000, Max Attempts:   50, Restarts :   5.]
#     num_queens = 100
#     fitness_cust = mlrose_hiive.CustomFitness(queens_max)
#     problem = mlrose_hiive.DiscreteOpt(length=num_queens, fitness_fn=fitness_cust, maximize=True, max_val=num_queens)
#     start = time.time()
...
#                                                                              restarts=5)
#     end = time.time()
#     print(f'Time taken to simulate RHC with optimized Params for {num_queens} Queens: {end-start}')
#
#     # SA  Best Params = [Fitness: 4938.0, Schedule: 'ExpDecay',  Attempt: 50, Initial_Temp: 1]
#     start = time.time()
#     best_state, best_fitness, fitness_curve = mlrose_hiive.simulated_annealing(problem=problem, curve=True, schedule=mlrose_hiive.ExpDecay(init_temp=1), max_attempts=50)
#     end = time.time()
#     print(f'Time taken to simulate SA with optimized Params for {num_queens} Queens: {end-start}')
#
#     # GA  Best Params = [Fitness: 4.94e+03, Mutation :  2.00e-01, Attempt : 3.00e+01, Population: 3.00e+02]
#     start = time.time()
#     best_state, best_fitness, fitness_curve = mlrose_hiive.genetic_alg(problem=problem, curve=True, pop_size=300, max_attempts=30, mutation_prob= 0.2)
#     end = time.time()
#     print(f'Time taken to simulate GA with optimized Params for {num_queens} Queens: {end-start}')
#
